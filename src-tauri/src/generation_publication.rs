...
fn receipt_name(reservation_id: &str) -> String {
    format!("{:x}.receipt.json", Sha256::digest(reservation_id.as_bytes()))
}

fn receipt_path(app_data: &Path, reservation_id: &str) -> PathBuf {
    app_data.join(RECEIPT_DIR).join(receipt_name(reservation_id))
}
...
fn file_link_count(path: &Path) -> Option<u64> {
    ...
    let succeeded = unsafe { GetFileInformationByHandle(file.as_raw_handle() as HANDLE, &raw mut info) };
    (succeeded != 0).then_some(u64::from(info.nNumberOfLinks))
}

#[cfg(not(any(unix, windows)))]
fn file_link_count(_path: &Path) -> Option<u64> { None }
...
fn recovery_blob(
    ...
    repo.protect_local_value(&json).map_err(|error| error.to_string())
}
...
fn decode_recovery_blob(
    ...
    let Some(stored) = receipt.recovery_blob.as_deref() else { return Ok(None); };
    let json = repo.unprotect_local_value(stored).map_err(|error| error.to_string())?;
    serde_json::from_str(&json).map(Some).map_err(|error| error.to_string())
}

fn hash_file(path: &Path, hasher: &mut Sha256) -> Result<(), String> {
    let bytes = std::fs::read(path).map_err(|error| format!("Не удалось прочитать результат для квитанции публикации: {error}"))?;
...
fn collect_files(root: &Path, current: &Path, files: &mut Vec<PathBuf>) -> Result<(), String> {
    ...
    for entry in entries {
        let path = entry.path();
        let ty = entry.file_type().map_err(|error| format!("Не удалось проверить тип файла комплекта: {error}"))?;
        if ty.is_symlink() { return Err("Комплект неожиданно содержит символическую ссылку.".into()); }
        if ty.is_dir() {
            collect_files(root, &path, files)?;
        } else if ty.is_file() {
            let relative = path.strip_prefix(root).map_err(|_| "Файл вышел за границы комплекта.".to_string())?;
...
fn completion_receipt_from_publication(receipt: &PublicationReceipt) -> Result<GenerationCompletionReceipt, String> {
    if receipt.effective_phase() != Some(PublicationPhase::Published) {
        return Err("Generation receipt допускается только после подтверждённой публикации.".into());
    }
    let plan_binding_sha256 = if receipt.has_complete_plan_binding() {
        Some(stable_identity_hash("plan-binding-v1", &[
    ...
    let receipt_id = stable_identity_hash("generation-receipt-v1", &[&receipt.reservation_id, &receipt.output_sha256]);
    let output_id = stable_identity_hash("generation-output-v1", &[&receipt.reservation_id, &receipt.output_sha256]);
...
fn completion_receipts_match_identity(existing: &GenerationCompletionReceipt, expected: &GenerationCompletionReceipt) -> bool {
...
fn persist_generation_completion_receipt(app_data: &Path, publication: &PublicationReceipt) -> Result<PathBuf, String> {
    let receipt = completion_receipt_from_publication(publication)?;
    let path = app_data.join(COMPLETION_RECEIPT_DIR).join(format!("{}.json", receipt.receipt_id));
    if path.exists() {
        let existing_bytes = std::fs::read(&path).map_err(|error| format!("Не удалось прочитать существующий committed GenerationReceipt: {error}"))?;
        let existing = serde_json::from_slice::<GenerationCompletionReceipt>(&existing_bytes)
            .map_err(|error| format!("Существующий committed GenerationReceipt повреждён: {error}"))?;
    ...
    crate::atomic_write_file(&path, &bytes).map_err(|error| format!("Не удалось зафиксировать committed GenerationReceipt: {error}"))?;
    Ok(path)
}
...
fn known_receipt_identity(receipt: &PublicationReceipt) -> bool {
    matches!(receipt.schema, LEGACY_RECEIPT_SCHEMA_V1 | LEGACY_RECEIPT_SCHEMA_V2 | RECEIPT_SCHEMA)
...
pub(crate) fn prepare_publication(
    ...
    let receipt = PublicationReceipt {
        ...
        published_unix: None,
        processing_job_sha256: binding.as_ref().map(|value| value.processing_job_sha256.clone()),
        source_sha256: binding.as_ref().map(|value| value.source_sha256.clone()),
        processing_fingerprint: binding.as_ref().map(|value| value.processing_fingerprint.clone()),
        recovery_blob: Some(recovery_blob(&repo, staged_output, counter_reservations)?),
    };
    write_receipt(&receipt_path(&app_data, &permit.reservation.reservation_id), &receipt)
}
...
pub(crate) fn attach_replacement_recovery(
    ...
    {
        return Err("Pre-publication квитанция не допускает привязку recovery безопасной замены.".into());
    }
    let state_path = crate::default_state_db_path(app)?;
    let repo = crate::repository_for(&state_path)?;
    ...
    let json = serde_json::to_string(&context).map_err(|error| error.to_string())?;
    receipt.recovery_blob = Some(repo.protect_local_value(&json).map_err(|error| error.to_string())?);
    write_receipt(&path, &receipt)
}

fn verify_published_output_digest(receipt: &PublicationReceipt, published_output: &Path) -> Result<(), String> {
    let published_sha256 = output_digest(published_output).map_err(|error| format!("Нельзя доказать целостность опубликованного результата: {error}"))?;
...
pub(crate) fn confirm_publication(
    ...
    use tauri::Manager as _;
    let app_data = app.path().app_data_dir().map_err(|error| format!("Не удалось получить app-data для проверки публикации: {error}"))?;
    let path = receipt_path(&app_data, &permit.reservation.reservation_id);
    let mut receipt = load_receipt(&path).map_err(|error| format!("Нельзя доказать целостность публикации: pre-publication квитанция недоступна: {error}"))?;
...
pub(crate) fn abort_prepared_publication(
    ...
    let path = receipt_path(&app_data, &permit.reservation.reservation_id);
    if path.exists() {
        std::fs::remove_file(path).map_err(|error| format!("Не удалось удалить pre-publication квитанцию: {error}"))?;
    }
    Ok(())
}

pub(crate) fn complete_publication_receipt(app: &tauri::AppHandle, permit: &crate::GenerationPermit) -> Result<(), String> {
    abort_prepared_publication(app, permit)
}

fn publication_receipt_for_permit(app_data: &Path, permit: &crate::GenerationPermit) -> Result<PublicationReceipt, String> {
    let receipt = load_receipt(&receipt_path(app_data, &permit.reservation.reservation_id))?;
    if !supported_receipt(&receipt) { return Err("Некорректная квитанция опубликованной генерации.".into()); }
    Ok(receipt)
}
...
pub(crate) fn finalize_published_generation(
    ...
    let publication_receipt = app_data.as_ref().map_err(|error| error.to_string()).and_then(|app_data| publication_receipt_for_permit(app_data, permit));
    ...
    let accounting_error = accounting_result.err().unwrap_or_else(|| "unknown accounting error".into());
    ...
    let _ = crate::create_automation_exception(app, "published_generation_accounting", "", &warning, &details);
    let _ = crate::append_audit_event(app, "published_generation_accounting_degraded", "", &details);
    vec![warning]
}
...
pub(crate) fn plan_bound_publication_guard_exists(
    ...
    let root_metadata = match std::fs::symlink_metadata(&root) {
        ...
        Err(error) => return Err(format!("Не удалось безопасно проверить каталог publication guards {}: {error}", root.display())),
    };
    if root_metadata.file_type().is_symlink() || !root_metadata.is_dir() {
        return Err(format!("Каталог publication guards имеет недопустимый тип: {}", root.display()));
    }
    let entries = std::fs::read_dir(&root).map_err(|error| format!("Не удалось прочитать каталог publication guards {}: {error}", root.display()))?;
    for entry in entries {
        let entry = entry.map_err(|error| format!("Не удалось прочитать запись каталога publication guards {}: {error}", root.display()))?;
        let path = entry.path();
        let metadata = std::fs::symlink_metadata(&path).map_err(|error| format!("Не удалось проверить publication guard {}: {error}", path.display()))?;
        if metadata.file_type().is_symlink() || !metadata.is_file() {
            return Err(format!("Publication guard имеет недопустимый тип: {}", path.display()));
        }
        let receipt = load_receipt(&path).map_err(|error| format!("Publication guard повреждён {}: {error}", path.display()))?;
        if !known_receipt_identity(&receipt) {
            return Err(format!("Publication guard имеет неизвестный или неполный формат: {}", path.display()));
        }
        if receipt.plan_binding_matches(processing_job_sha256, source_sha256, processing_fingerprint) {
            return Ok(true);
        }
    }
    Ok(false)
...
fn restore_interrupted_replacement(
    ...
    let (Some(target_raw), Some(backup_raw)) = (context.replacement_target.as_deref(), context.replacement_backup.as_deref()) else { return Ok(()); };
    let target = Path::new(target_raw);
    let backup = Path::new(backup_raw);
    let target_parent = target.parent().ok_or_else(|| "Recovery-путь публикации не имеет родительской папки.".to_string())?;
    ...
    let metadata = std::fs::symlink_metadata(backup).map_err(|error| format!("Не удалось проверить резервную копию после сбоя: {error}"))?;
    ...
    std::fs::create_dir_all(target_parent).map_err(|error| format!("Не удалось подготовить папку для восстановления backup: {error}"))?;
    std::fs::rename(backup, target).map_err(|error| format!("Не удалось восстановить предыдущий комплект после сбоя: {error}"))?;
...
fn rollback_unpublished_receipt(
    ...
    if !staged_output_definitely_unpublished(context) { return Ok(false); }
    restore_interrupted_replacement(context, report)?;
    let reservation = UsageReservation {
        reservation_id: receipt.reservation_id.clone(), month_key: String::new(), documents: 0, trial: false,
    };
    if !repo.rollback_usage(&reservation).map_err(|error| error.to_string())? { return Ok(false); }
    let mut counter_gaps = Vec::new();
    for counter in context.counter_reservations.iter().rev() {
        match repo.rollback_counter(counter) {
            Ok(true) => {}
            Ok(false) => counter_gaps.push(format!("{}:{}={}", counter.counter_key, counter.year, counter.value)),
            Err(error) => counter_gaps.push(format!("{}:{}={} ({error})", counter.counter_key, counter.year, counter.value)),
        }
    }
...
pub(crate) fn reconcile_publication_receipts(
    ...
    for entry in entries.flatten() {
        ...
        let receipt = match load_receipt(&path) {
            ...
            Ok(_) => { report.warnings.push("Некорректная квитанция опубликованной генерации оставлена для ручной проверки.".into()); continue; }
            Err(_) => { report.warnings.push("Повреждённая квитанция опубликованной генерации оставлена для ручной проверки.".into()); continue; }
        };
        if receipt.effective_phase() == Some(PublicationPhase::Prepared) && receipt.schema == RECEIPT_SCHEMA {
...
fn recover_stale_prepublication_reservations(
    ...
    let stale = repo.stale_publication_recovery_reservations(24 * 60).map_err(|error| error.to_string())?;
    ...
    for reservation in stale {
        ...
        match std::fs::symlink_metadata(&path) {
            ...
            Err(error) => return Err(format!("Не удалось безопасно проверить publication receipt {}: {error}", path.display())),
        }
        if repo.rollback_usage(&reservation).map_err(|error| error.to_string())? { rolled_back += 1; }
    }
    Ok(rolled_back)
}
...
pub(crate) fn recover_startup_generation_state(app: &tauri::AppHandle, repo: &mut LocalRepository) {
    ...
    if let Ok(app_data) = app.path().app_data_dir() {
        ...
        if report.finalized > 0 { eprintln!("Восстановлен учёт {} опубликованных генераций после сбоя.", report.finalized); }
        if report.rolled_back > 0 { eprintln!("Отменено {} доказанно неопубликованных генераций после сбоя; лимит возвращён.", report.rolled_back); }
        if report.ambiguous > 0 { eprintln!("Обнаружено {} двусмысленных pre-publication состояний; повтор заблокирован до ручной проверки.", report.ambiguous); }
        for warning in report.warnings { eprintln!("Восстановление опубликованной генерации: {warning}"); }
        match recover_stale_prepublication_reservations(&app_data, repo) {
            Ok(count) if count > 0 => eprintln!("Возвращён лимит для {count} зависших v3-резерваций без publication receipt."),
            Ok(_) => {}
            Err(error) => eprintln!("Не удалось безопасно восстановить v3-резервации до publication receipt: {error}"),
        }
    }
    if let Err(error) = repo.recover_stale_usage_reservations(24 * 60) { eprintln!("Не удалось восстановить зависшие резервации лимита: {error}"); }
    if let Err(error) = repo.recover_interrupted_case_runs() { eprintln!("Не удалось восстановить прерванные дела: {error}"); }
}
...
mod tests {
    ...
    fn committed_generation_receipt_is_non_pii_and_bound_to_output() {
        ...
        let completion = completion_receipt_from_publication(&publication).expect("published receipt creates committed completion proof");
    ...
    fn committed_generation_receipt_is_idempotent_and_conflict_safe() {
        ...
        let path = persist_generation_completion_receipt(&root, &publication).expect("first committed receipt write");
        let mut existing: GenerationCompletionReceipt = serde_json::from_slice(&std::fs::read(&path).unwrap()).unwrap();
        ...
        let replayed = persist_generation_completion_receipt(&root, &publication).expect("identical replay must reuse committed receipt");
        assert_eq!(replayed, path);
        assert_eq!(std::fs::read(&path).unwrap(), stable_bytes);
        let mut conflicting: GenerationCompletionReceipt = serde_json::from_slice(&stable_bytes).unwrap();
        conflicting.output_id = "0".repeat(64);
        std::fs::write(&path, serde_json::to_vec_pretty(&conflicting).unwrap()).unwrap();
        let error = persist_generation_completion_receipt(&root, &publication).expect_err("conflicting committed receipt must fail closed");
    ...
    fn ambiguous_phase_still_blocks_same_plan_automatic_retry() {
        ...
        assert!(!plan_bound_publication_guard_exists(&root, "other-job", "source", "plan").unwrap());
        let _ = std::fs::remove_dir_all(root);
    }
    ...
    fn corrupt_local_completion_guard_fails_closed() {
        ...
        let error = local_completion_receipt_matches(&root, "job", "source", "plan").expect_err("corrupt completion guard must not mean not-completed");
    ...
    fn unreadable_publication_guard_namespace_fails_closed() {
        ...
        let error = plan_bound_publication_guard_exists(&root, "job", "source", "plan").expect_err("invalid receipt namespace must stop automatic issuance");
    ...
    fn malformed_publication_receipt_fails_closed() {
        ...
        let error = plan_bound_publication_guard_exists(&root, "job", "source", "plan").expect_err("malformed publication receipt must stop automatic issuance");
    ...
    fn publication_digest_proof_rejects_any_post_prepare_change() {
        ...
        let error = verify_published_output_digest(&receipt, &published).expect_err("changed published bytes must never confirm");
        assert!(error.contains("не совпал с подготовленным snapshot"), "{error}");
        let _ = std::fs::remove_dir_all(root);
    }
    ...
    fn unverified_emergency_guard_is_plan_bound_and_blocks_only_same_job() {
        ...
        std::fs::write(&source, b"source").unwrap();
        let job = "a".repeat(64);
        mark_plan_bound_emergency_guard(&source, &"b".repeat(64), &job, "unverified_publication_quarantined").unwrap();
        assert!(plan_bound_emergency_completion_exists(&source, &job).unwrap());
        assert!(!plan_bound_emergency_completion_exists(&source, &"c".repeat(64)).unwrap());
        assert!(mark_plan_bound_emergency_guard(&source, &"b".repeat(64), &job, "retryable").is_err());
        let _ = std::fs::remove_dir_all(root);
    }

    fn temp_root(label: &str) -> PathBuf {
        std::env::temp_dir().join(format!("dokkomplekt-publication-{label}-{}-{}", std::process::id(), uuid::Uuid::new_v4()))
    }
    ...
    fn interrupted_replacement_never_follows_symlinked_backup_root() {
        ...
        let mut report = PublicationReconciliationReport::default();
        let error = restore_interrupted_replacement(&context, &mut report).expect_err("recovery must not traverse symlinked backup root");
