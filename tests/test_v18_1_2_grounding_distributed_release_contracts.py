...
class V1812GroundingDistributedReleaseContracts(unittest.TestCase):
    def test_version_is_synchronized_and_rust_marker_is_not_forged(self) -> None:
        self.assertEqual(read("VERSION").strip(), "18.4.5")
        self.assertEqual(json.loads(read("package.json"))["version"], "18.4.5")
        self.assertEqual(json.loads(read("src-tauri/tauri.conf.json"))["version"], "18.4.5")
