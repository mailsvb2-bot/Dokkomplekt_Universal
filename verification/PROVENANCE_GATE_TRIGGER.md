...
PR #6 verifies the profession-neutral client-first UI after the runner-generated source manifest was synchronized, including text selection, one-click correction, bounded unreadable-file feedback, the neutral exchange-package action, a deterministic neutral dictionary fixture for Vitest, client-readable language throughout advanced settings and automation metrics, retained machine-readable Python contract evidence on every CI outcome, honest time-savings language tied to real measurements, machine-readable Vitest evidence on every UI test outcome, and source replacement from the already accepted state. This trigger applies the final accepted-state test correction after prior CI runs settled.
