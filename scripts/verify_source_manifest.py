#!/usr/bin/env python3
"""One-shot, checksum-pinned publisher for the verified UI simplification patch."""
...
BASE_SHA = "c76069bba6103a574921187ddeca30e65e7fa265"
PATCH_SHA256 = "86fdcfb6788327f51c3df3fb7552f90bf28b50ecabf743d8e9ae5ceedc88202c"
CHUNK_DIR = ROOT / "verification" / "ui-bootstrap"
...
def bootstrap() -> int:
    ...
    allowed.update(str(path.relative_to(ROOT)).replace("\\", "/") for path in CHUNK_DIR.glob("part-*.txt"))
    ...
    if not chunks:
        raise RuntimeError("bootstrap patch chunks are missing")
    ...
    patch_path = ROOT / ".ui-simplification.patch"
    ...
    run("git", "add", "-A")
    run("git", "commit", "-m", "Improve daily document workflow and settings UX")
