...
def main() -> int:
    ...
    if before != expected_before:
        raise RuntimeError(f"rustfmt changed unexpected paths: {sorted(before)}")
    ...
    if final != expected_final:
        raise RuntimeError(f"repair produced unexpected paths: {sorted(final)}")
    ...
    run("git", "commit", "-m", "style(rust): format native folder picker test")
