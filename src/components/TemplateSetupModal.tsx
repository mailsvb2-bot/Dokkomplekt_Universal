...
export function TemplateSetupModal(props: TemplateSetupModalProps) {
  ...
  return (
    <div className="backdrop" role="dialog" aria-modal="true" aria-label="Добавление шаблонов">
      <div className="modal fileDropZone templateSetupWide" onDragOver={(event: DragEvent<HTMLDivElement>) => event.preventDefault()} onDrop={(event: DragEvent<HTMLDivElement>) => {
        event.preventDefault();
        const files = Array.from(event.dataTransfer.files ?? []);
        if (files.length) props.onDropFiles(files);
      }}>
        <h2>Создать кнопки документов</h2>
        <p className="hint">Выберите свои DOCX или DOCM. Название каждого документа станет кнопкой на главном экране.</p>
        ...
        {!hasBatch ? (
          <div className="emptyPackage templateFirstStep">
            ...
            <details className="manualScannerDetails">
              ...
              <textarea value={props.templateText} onChange={(event) => props.onTemplateTextChange(event.target.value)} onSelect={(event) => {
                const target = event.currentTarget;
                const start = target.selectionStart ?? 0;
                const end = target.selectionEnd ?? start;
                setSelection(end > start ? { start, end, text: target.value.slice(start, end) } : null);
              }} spellCheck={false} placeholder="Вставьте текст документа" />
              <table className="confirm"><tbody><tr><th>Документ</th><td>{props.previewTitle}</td></tr><tr><th>Название кнопки</th><td><input value={props.buttonLabel} placeholder={props.previewTitle} onChange={(event) => props.onButtonLabelChange(event.target.value)} /></td></tr></tbody></table>
              <button className="softBtn" onClick={props.onAnalyze}>Анализировать</button>
              <details className="manualScannerDetails"><summary>Дополнительная разметка</summary><ScannerToolbar selection={selection?.text ?? ''} fieldId={scannerField} onFieldIdChange={setScannerField} onReplace={() => applyVisualMarkup('replace')} onInsert={() => applyVisualMarkup('insert_after')} /><PopupFieldEditor compact fields={props.draftPopupFields} onChange={props.onDraftPopupFieldsChange} /></details>
            </details>
          </div>
        ) : (
          <>
            <h3 className="templateBatchHead">2. Проверьте названия кнопок</h3>
            <div className="templateBatch" aria-label="Подготовленные шаблоны">
              {props.pendingTemplates.map((item) => (
                <div className={activePending?.document_id === item.document_id ? 'templateBatchRow selected' : 'templateBatchRow'} key={item.document_id}>
                  <button className="templateFileSelect" type="button" title="Открыть дополнительную настройку" onClick={() => { setActivePendingId(item.document_id); setSelection(null); }}>{item.file_name}</button>
            ...
            <div className="readyMessage templateReadyMessage"><i className="ti ti-circle-check" aria-hidden="true" /><div><strong>Всё готово</strong><span>Нажмите «{confirmLabel}». Обычные шаблоны без специальных полей тоже будут добавлены.</span></div></div>
            {activePending ? (
              <details className="manualScannerDetails templateAdvancedSetup">
                ...
                <div className="pendingCursorScanner">
                  <div className="guidedTemplateLaunch"><div><strong>Показать места для автоматического заполнения</strong><small>Это необязательно. Кнопка документа создаётся и без разметки.</small></div><button className="softBtn" type="button" onClick={() => props.onStartGuidedPendingScanner(activePending.document_id)}><i className="ti ti-hand-click" aria-hidden="true" /> Открыть Word и показать место</button></div>
                  <details className="manualScannerDetails"><summary>Ручная разметка для опытных пользователей</summary><textarea value={activePending.extracted_text} readOnly aria-label={`Текст шаблона ${activePending.file_name} для разметки`} onSelect={(event) => {
                    const target = event.currentTarget;
                    const start = target.selectionStart ?? 0;
                    const end = target.selectionEnd ?? start;
                    setSelection(end > start ? { start, end, text: target.value.slice(start, end) } : null);
                  }} spellCheck={false} /><ScannerToolbar selection={selection?.text ?? ''} fieldId={scannerField} onFieldIdChange={setScannerField} disabled={marking} onReplace={() => void applyPendingVisualMarkup('replace')} onInsert={() => void applyPendingVisualMarkup('insert_after')} /></details>
        ...
        <datalist id="template-scanner-field-suggestions"><option value="document.number" /><option value="document.date" /><option value="subject.name" /><option value="org.name" /><option value="amount.total" /><option value="period.start_date" /><option value="period.end_date" /><option value="related.number" /></datalist>
        <div className="modalActions"><span className="spacer" /><button className="softBtn" onClick={props.onCancel}>Закрыть</button><button className="primaryBtn" onClick={props.onConfirm} disabled={!hasBatch && !props.templateText.trim()}>{confirmLabel}</button></div>
      </div>
    </div>
  );
}

function ScannerToolbar(props: { selection: string; fieldId: string; disabled?: boolean; onFieldIdChange(value: string): void; onReplace(): void; onInsert(): void; }) {
  return <div className="templateScanner"><div className="scannerSelection"><i className="ti ti-cursor-text" aria-hidden="true" />{props.selection ? `Выделено: ${props.selection}` : 'Выделите мышкой примерное значение'}</div><input value={props.fieldId} onChange={(event) => props.onFieldIdChange(event.target.value)} placeholder="поле, например document.number" aria-label="Смысловое поле сканера" list="template-scanner-field-suggestions" /><button className="softBtn" type="button" onClick={props.onReplace} disabled={props.disabled || !props.selection || !props.fieldId.trim()}>Заменить</button><button className="softBtn" type="button" onClick={props.onInsert} disabled={props.disabled || !props.selection || !props.fieldId.trim()}>Вставить после</button></div>;
}
