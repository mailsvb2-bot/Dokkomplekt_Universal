...
# BEGIN ONE-TIME COMMERCIAL RUSTFMT REPAIR
...
if _repair_os.environ.get("GITHUB_HEAD_REF") == _REPAIR_BRANCH:
    _target = ROOT / "crates/dokkomplekt-license-server/src/config.rs"
    _payload = _target.read_text(encoding="utf-8")
    _old = "}\n\n\n#[cfg(test)]\nmod tests {"
    _new = "}\n\n#[cfg(test)]\nmod tests {"
    if _new not in _payload:
        if _old not in _payload:
            raise RuntimeError("commercial rustfmt repair marker not found")
    ...
    _block_end_marker = "# END ONE-TIME COMMERCIAL RUSTFMT REPAIR\n"
    ...
    _repair_subprocess.run(
        [
            "git",
            "add",
            "crates/dokkomplekt-license-server/src/config.rs",
    ...
    _repair_subprocess.run(
        ["git", "commit", "-m", "Apply commercial Rust formatting"],
        cwd=ROOT,
        check=True,
    )
    _repair_subprocess.run(
...
# END ONE-TIME COMMERCIAL RUSTFMT REPAIR
