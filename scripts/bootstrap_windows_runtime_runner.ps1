...
function Assert-Administrator {
    ...
    if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) { throw 'Run runtime runner bootstrap from an elevated PowerShell window.' }
}
...
function Ensure-OpenSsl {
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $opensslDir 'openssl.exe') -PathType Leaf)) { throw 'OpenSSL was not found in Git for Windows.' }
...
function Assert-RuntimeManifest([string] $Path) {
    ...
    if ([int]$manifest.schema -ne 1 -or [string]$manifest.target -ne 'windows-x86_64' -or $manifest.supply_chain_locked -ne $true) { throw 'Runtime manifest must be schema=1, target=windows-x86_64 and supply_chain_locked=true.' }
...
function Assert-ServiceAclEvidence([string] $ManifestPath,[string] $BoundedRuntimeRoot) {
    $rootItem = Get-Item -LiteralPath $BoundedRuntimeRoot -Force -ErrorAction Stop
    if (-not $rootItem.PSIsContainer -or (($rootItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0)) { throw 'RuntimeRoot must be a direct directory.' }
    $root = $rootItem.FullName.TrimEnd('\')
    $manifest = [IO.Path]::GetFullPath($ManifestPath)
    if ($manifest -ine $root -and -not $manifest.StartsWith($root + '\',[StringComparison]::OrdinalIgnoreCase)) { throw "Runtime manifest must be under bounded service root '$root'." }
    if (-not (Test-Path -LiteralPath $AclEvidencePath -PathType Leaf)) { throw "Runtime service ACL evidence is missing: $AclEvidencePath. Run register_windows_runtime_runner.ps1 instead of bypassing the registration entrypoint." }
    $record = Get-Content -LiteralPath $AclEvidencePath -Raw | ConvertFrom-Json
    if ([string]$record.schema -ne 'dokkomplekt.runtime-service-acl.v2') { throw 'Runtime service ACL evidence schema mismatch.' }
    ...
    if ([string]$record.service_sid -ne $ServiceSid -or [string]$record.access -ne 'ReadAndExecute' -or $record.recursive_acl_applied -ne $true) { throw 'Runtime service ACL evidence does not prove bounded Network Service read/execute access.' }
    $acl = Get-Acl -LiteralPath $root
    $rule = @($acl.Access | Where-Object { (Get-RuleSid $_) -eq $ServiceSid -and ($_.FileSystemRights -band [Security.AccessControl.FileSystemRights]::ReadAndExecute) })
    if ($rule.Count -eq 0) { throw 'Bounded runtime root no longer grants Network Service SID ReadAndExecute.' }
...
try {
    & .\config.cmd --unattended --url $RepositoryUrl --token $RegistrationToken --name $RunnerName --labels $RunnerLabel --work _work --replace --runasservice --windowslogonaccount $ServiceAccount
...
[ordered]@{
    schema='dokkomplekt.runtime-runner-bootstrap.v3'
