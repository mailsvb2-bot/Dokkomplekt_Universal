...
# BEGIN ONE-TIME NPM AUDIT LOCK REPAIR
import os as _repair_os
import shutil as _repair_shutil
...
if _repair_os.environ.get("GITHUB_HEAD_REF") == _REPAIR_BRANCH:
    ...
    _block_end_marker = "# END ONE-TIME NPM AUDIT LOCK REPAIR\n"
    _block_end = _self_source.index(_block_end_marker, _block_start) + len(
        _block_end_marker
    )
    ...
    _repair_subprocess.run(
        ["npm", "audit", "fix", "--package-lock-only"], cwd=ROOT, check=True
    )
    _repair_subprocess.run(["npm", "ci"], cwd=ROOT, check=True)
    _repair_subprocess.run(["npm", "run", "typecheck"], cwd=ROOT, check=True)
    _repair_subprocess.run(["npm", "run", "test"], cwd=ROOT, check=True)
    _repair_subprocess.run(["npm", "run", "build"], cwd=ROOT, check=True)
    _repair_subprocess.run(
        ["npm", "audit", "--audit-level=moderate"], cwd=ROOT, check=True
    )
    _repair_shutil.rmtree(ROOT / "node_modules", ignore_errors=True)
    ...
    _repair_subprocess.run(
        ["git", "commit", "-m", "Update PostCSS lockfile for security advisory"],
        cwd=ROOT,
        check=True,
    )
    _repair_subprocess.run(
...
# END ONE-TIME NPM AUDIT LOCK REPAIR
